...
pairMaybe' :: Maybe a -> Maybe b -> Maybe (a, b)
pairMaybe' x y = liftMaybe ( \ p q -> (p, q) ) x y
...
lookups [] _ = Just []
lookups _ [] = Nothing 
lookups ( x : xs) table  = case ( lookup x table ) of 
                              Nothing ->  Nothing 
                              Just y -> do
                                res <- lookups xs table 
                                Just $ y : res

  
  
-- lookups ( x : xs) ( (key,val) : table) = if ( x == key )
--                                           then val : ( lookups  xs table )
--                                         else
--                                            lookups ( x : xs ) table 
...
reverseAcc :: [a] -> [a] -> [a]
reverseAcc l [] = l
reverseAcc ls  ( y : ys ) =  reverseAcc  ( y : ls )  ys 
...
-- than the other? Why?
--
-- reverse' will be faster because this function is just appending element 
-- to a list which is initially empty but reverse will recursively go through all
-- the elements and ++ operator append it to the output list
...
sameShape' :: Tree a -> Tree b -> Bool
-- sameShape' ( Node  l1 r1 ) ( Node l2 r2 ) = ( Node l1 () == Node l2 () ) && ( Node r1 () == Node r2 () )
sameShape' (a) (b) = (mapTree (const())  a) == (mapTree (const())  b)
-- Task B-17.
--
...
buildTree 0 = Leaf ()
buildTree x = Node ( buildTree ( x-1 ) ) (buildTree ( x-1 ))
-- Task B-18.
--
...
eval :: Expr -> Int
-- eval = error "TODO: implement countOps"

eval  (Lit x)  = x
eval (Add e1 e2 ) = eval e1 +  eval e2 
eval (Neg e) = - eval e 
eval (IfZero p q r) = if ( eval p == 0 )
                          then eval q
                           else
                            eval r 
...
countOps (IfZero p q r) = 1 + ( countOps p ) + ( countOps q) + ( countOps r )
...
data Path  = End | Lft Path | Rght Path
  deriving (Eq, Show)
...
follow End t = case t of 
              Leaf x -> Just x
              _ -> Nothing

follow (Lft p) t   =      case t of  
                    Leaf _ -> Nothing
                    Node l _ -> follow p l    

follow (Rght p) t   =    case t of  
                    Leaf _ -> Nothing
                    Node _ r -> follow p r                         
...
search x (Node a b) = case search x a of 
                      Just p -> Just(Lft p)
                      Nothing -> case search x b of
                                   Just p -> Just(Rght p)
                                   Nothing -> Nothing
