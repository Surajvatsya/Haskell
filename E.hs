...
-- When calling "unsafeReadInt" with a string that can be parsed as an integer, 
-- it will successfully return the integer value. while calling unsafeReadInt "3as3" gives
...
sumTwo :: IO ()
sumTwo = do
    n1 <- askInt "Please enter first number:"
...
replicateM n ioa = sequence ( replicate n ioa )
...
sumMany = do 
    ...
    mp <- replicateM n  ( askInt "Enter next number:" )
    let ans = show ( sum mp )
    putStrLn ans
