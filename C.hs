...
countWhiteSpaceStrings = length . filter ( \x -> all isSpace x  )
...
removeLongWords n = unwords. filter ( \x ->  length x  <= n ).words 
...
goDir :: Pos -> Dir -> Pos
goDir (MkPos pUp pRight) dir = case dir of
  Up -> MkPos (pUp+1) pRight
  Rgt -> MkPos pUp (pRight+1)
  Down -> MkPos (pUp-1) pRight
  Lft -> MkPos pUp (pRight-1)
