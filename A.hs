...
-- null3 xs = length xs == 0
 ...
 --because 
 -- null2 => verifying boolean values is less convenient comparatively pattern matching
 -- null3 => will check the whole length of list recursively . 
...
sumEvenSquares [] = 0
sumEvenSquares ( l : ls ) = if ( l `mod` 2 == 0 ) 
                              then l*l + sumEvenSquares ls
                          else
                            sumEvenSquares ls
...
allEven :: [Int] -> Bool
allEven [] = True
allEven ( l : ls ) = l `mod` 2 == 0 && allEven ls 
...
isAscending []  = True
isAscending [x]  = True
isAscending ( l1 : ( l2 : ls ) ) = l1 < l2 && isAscending ( l2 : ls )
